		...
		if ($resultSecond=mysqli_query($con,$sqlProducts)) {
			while ($row2=mysqli_fetch_row($resultSecond))
			{
				if($row[0]=$row2[0]) {
					...
					$pInStorage = "";
					if($pcount>0) {
						$pInStorage = "ÅÑÒÜ ÍÀ ÑÊËÀÄÅ";
					} else {
						$pInStorage = "ÍÅÒ ÍÀ ÑÊËÀÄÅ";
					}
					...
					if($pdiscount>0) {
						...
						<tr>
							<td align=center height=200pt id=\"WatchDiscounts_".$pid."\" onclick=\"WatchProduct(this.id);\">
							<hr>
							<span>".$pname."</span>
							<hr>
							<span>ÊÎÄ: ".$pid."</span>
							<br>
							<span>ÍÀËÈ×ÈÅ: ".$pInStorage."</span>
							<hr>
							</td>
						</tr>
						...
						<tr>
							...
							<input id=\"InputDiscounts_".$pid."\" class=\"CountProductInput\" size=\"1\" type=\"number\" value=\"1\" min=\"1\" size=\"1\" align=\"left\" />
							</td>
							<td align=\"center\">
							<span id=\"SpanDiscounts_".$pid."\" class=\"SendToCartButton\" onclick=\"AddToCart(this.id);\">Â êîðçèíó</span>
							</td>
							</tr></table>
						...
						if($pdiscount>0) {
							...
							echo "
								<span><strike>".$pprice."</strike> ".$pdiscountNow. " ðóá.</span>
							";
						} else {
						echo "
							<span>".$pprice." ðóá.</span>
						";
						}
						echo "
							</td>
